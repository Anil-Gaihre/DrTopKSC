...
{
	cout<<"./exe num_element k NBucket scoreFile output_file"<<endl;
	...
	if ((fptr = fopen(scr_file,"r")) == NULL)
{
 ...
 char * str;
 fscanf(fptr,"%s", str);
 int num_element1 = num_element/2;
for (index_t i=0;i<num_element1;i++)
{
	fscanf(fptr,"%d", &vec[i]);
	vec[i] = (1-vec[i])*1000000000;
	vec1[i]=vec[i];
	vec[i+num_element1] = (1-vec[i])*1000000000;
	vec1[i+num_element1]=vec[i+num_element1];
}
// int offset = num_element1;
// int offset_j=0;
...
// }
	...
	timeLog.open(argv[5],std::fstream::out | std::fstream::app);
