...
{
	cout<<"./exe num_element k NBitsPerDigit beta const_tuning"<<endl;
	...
	index_t const_tuning=atol(argv[5]);
	...
	index_t alpha=0.5*(num_pow-log(k)/log(2)+const_tuning);
	...
	//   for (int dis=3;dis<4;dis++)
	{
		...
		int minvalue=2147483643;
		// std::uniform_int_distribution <unsigned int> d_Lower(0, 100000000);//(0 to 100 million)
		...
		//    for (int Kiteration=atol(argv[2]);Kiteration<536870912;Kiteration=Kiteration*2)
		{
			...
			timeLog.open("Const_seclectionVaryK_N_30.csv",std::fstream::out | std::fstream::app);
			timeLog<<const_tuning<<";";
			if  (true)
			{
				timeLog<<"D"<<";";
			}
