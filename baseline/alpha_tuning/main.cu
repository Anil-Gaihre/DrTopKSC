...
{
	cout<<"./exe num_element k NBitsPerDigit beta alpha"<<endl;
	...
	//   for (int dis=3;dis<4;dis++)
	{
		...
		//    for (int Kiteration=atol(argv[2]);Kiteration<536870912;Kiteration=Kiteration*2)
		{
			...
			timeLog.open("U_alpha_tuning_test_2^30.csv",std::fstream::out | std::fstream::app);
			if  (true)
			{
				timeLog<<"D"<<";";
			}
