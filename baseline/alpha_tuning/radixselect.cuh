...
template<typename data_t, typename index_t>
__global__ void CreateConcatenateRangeNoOptimization(data_t* vec,data_t* ConcatenatedRange,index_t* SubrangeId,index_t ConcatenatedSize,index_t k,index_t Subrangesize,index_t* write_pos,data_t firsttopk)
{
	index_t thid = blockDim.x*blockIdx.x+threadIdx.x;
	index_t laneId= threadIdx.x & 0x1f;
	index_t myWarpID=thid >> 5;
		...
		TotalBlocksrequired,int SizeOfAllocation,int NThreadsPerBlock,int beta,bool defaultContribution,int NthreadstoworkInreduction,index_t* SelectedSubrangeId_d,index_t*CountLonelyElements_d,index_t* write_pos_d,data_t* ConcatenatedRange_d,index_t* CountSelectedSubrange_d)
{
	...
	if (true)
	{
		...
		// CreateConcatenateRangeNoOptimization<data_t,index_t><<<512,512>>>(vec_d,ConcatenatedRange_d,SelectedSubrangeId_d,ConcatenatedSize,k,SubRangesize,write_pos_d,TopKElement);
	...
	// H_ERR(cudaMemcpy(&ConcatenatedSize,k*SubRangesize,sizeof(index_t),cudaMemcpyDeviceToHost));
