...
template<typename data_t,typename index_t>
__global__ void sampleMax_Beta_WithParallelReduction2 (data_t* A,data_t* SubRangeMax, index_t N,index_t NSubranges,index_t SubRangeSize,const index_t alpha,index_t* SubrangeId,int NSharedMememoryElements,int beta)
{
	int thid = blockDim.x*blockIdx.x+threadIdx.x;
	int laneId= threadIdx.x & 0x1f;
	int myWarpID=thid >> 5;
	int NWarps=(blockDim.x*gridDim.x) >> 5;
	int pos = myWarpID*beta;
	while (myWarpID < NSubranges)//WarpID is used as subrange ID
	{
		index_t mybegin_pos=(myWarpID<<alpha)+laneId;
		index_t myend_pos=(((myWarpID+1)<<alpha) < (N)) ? ((myWarpID+1)<<alpha):N;
		...
		data_t MaxFromOther;
		data_t SecondMaxFromOther;
		int Nthreadstowork =32;
		for (int j=Nthreadstowork >> 1;j >=1;j=j>>1)
		{
			MaxFromOther=__shfl_sync(0xffffffff, Max,laneId+j,32/*Nthreadstowork*//*32*/);
			SecondMaxFromOther=__shfl_sync(0xffffffff, SecondMax,laneId+j,32/*Nthreadstowork*//*32*/);
			if (laneId<j)
			{
				if (Max > MaxFromOther)
				{
					if (SecondMax > MaxFromOther)
					{
						//do nothing
					}
					else
					{
						//secondMax is smaller/equal than MaxFromOther
						SecondMax = MaxFromOther;
					}
				}
				else
				{
					//max is smaller thn maxfrom other
					if (Max > SecondMaxFromOther)
					{
						SecondMax = Max;
						Max = MaxFromOther;
					}
					...
					{
						SecondMax = SecondMaxFromOther;
						Max = MaxFromOther;
					}
				}
				// Max= (MaxFromOther > Max) ? MaxFromOther : Max ;
				// SecondMax= (SecondMaxFromOther > SecondMax) ? SecondMaxFromOther : SecondMax ;
			}            
		}
		if(laneId==0)
		{
			SubRangeMax[pos]=Max;
			SubrangeId[pos]=myWarpID;

			SubRangeMax[pos+1]=SecondMax;
			SubrangeId[pos+1]=myWarpID;
		}
		myWarpID+=NWarps;
		pos = myWarpID*beta;
	}
...
template<typename data_t,typename index_t>
__global__ void sampleMax_Beta_WithParallelReduction3 (data_t* A,data_t* SubRangeMax, index_t N,index_t NSubranges,index_t SubRangeSize,const index_t alpha,index_t* SubrangeId,int NSharedMememoryElements,int beta)
{
	int thid = blockDim.x*blockIdx.x+threadIdx.x;
	int laneId= threadIdx.x & 0x1f;
	int myWarpID=thid >> 5;
	...
	while (myWarpID < NSubranges)//WarpID is used as subrange ID
	{
		...
		data_t MaxFromOther;
		data_t SecondMaxFromOther;
		data_t ThirdMaxFromOther;
		int Nthreadstowork =32;
		for (int j=Nthreadstowork >> 1;j >=1;j=j>>1)
		{
			MaxFromOther=__shfl_sync(0xffffffff, Max,laneId+j,32/*Nthreadstowork*//*32*/);
			SecondMaxFromOther=__shfl_sync(0xffffffff, SecondMax,laneId+j,32/*Nthreadstowork*//*32*/);
			ThirdMaxFromOther=__shfl_sync(0xffffffff, ThirdMax,laneId+j,32/*Nthreadstowork*//*32*/);
			if (laneId<j)
			{
				Max= (MaxFromOther > Max) ? MaxFromOther : Max ;
				SecondMax= (SecondMaxFromOther > SecondMax) ? SecondMaxFromOther : SecondMax ;
				ThirdMax= (ThirdMaxFromOther > ThirdMax) ? ThirdMaxFromOther : ThirdMax ;
			}            
		}
		if(laneId==0)
		{
			...
			SubRangeMax[pos+2]=ThirdMax;
			SubrangeId[pos+2]=myWarpID;
		}
		myWarpID+=NWarps;
		pos = myWarpID*beta;
	}
...
template<typename data_t,typename index_t>
__global__ void sampleMax_Beta_WithParallelReduction4 (data_t* A,data_t* SubRangeMax, index_t N,index_t NSubranges,index_t SubRangeSize,const index_t alpha,index_t* SubrangeId,int NSharedMememoryElements,int beta)
{
	int thid = blockDim.x*blockIdx.x+threadIdx.x;
	int laneId= threadIdx.x & 0x1f;
	int myWarpID=thid >> 5;
	...
	while (myWarpID < NSubranges)//WarpID is used as subrange ID
	{
		...
		for (int j=Nthreadstowork >> 1;j >=1;j=j>>1)
		{
			...
			SecondMaxFromOther=__shfl_sync(0xffffffff, SecondMax,laneId+j,32/*Nthreadstowork*//*32*/);
			ThirdMaxFromOther=__shfl_sync(0xffffffff, ThirdMax,laneId+j,32/*Nthreadstowork*//*32*/);
			FourthMaxFromOther=__shfl_sync(0xffffffff, FourthMax,laneId+j,32/*Nthreadstowork*//*32*/);
			if (laneId<j)
			{
				Max= (MaxFromOther > Max) ? MaxFromOther : Max ;
				SecondMax= (SecondMaxFromOther > SecondMax) ? SecondMaxFromOther : SecondMax ;
				ThirdMax= (ThirdMaxFromOther > ThirdMax) ? ThirdMaxFromOther : ThirdMax ;
				FourthMax= (FourthMaxFromOther > FourthMax) ? FourthMaxFromOther : FourthMax ;
			}            
		}
		if(laneId==0)
		{
			...
			SubRangeMax[pos+3]=FourthMax;
			SubrangeId[pos+3]=myWarpID;
		}
		myWarpID+=NWarps;
		pos = myWarpID*beta;
	}
		...
		TotalBlocksrequired,int SizeOfAllocation,int NThreadsPerBlock,int beta,bool defaultContribution,int NthreadstoworkInreduction,index_t* SelectedSubrangeId_d,index_t*CountLonelyElements_d,index_t* write_pos_d,data_t* ConcatenatedRange_d,index_t* CountSelectedSubrange_d)
{
	...
	else
	{
		if (beta ==2)
		{
			//Beta sampling functions without shared memory optimization don't give right result. So Don't use for evaluation
			...
			sampleMax_Beta_WithParallelReduction2 <data_t,index_t><<<4096,512>>>(vec_d,Max_d,num_element,NSubranges,SubRangesize,alpha,SubrangeId_d,NSharedMemoryElements,beta);
		}
		else if (beta ==3)
		{
			//case beta =2
			...
			sampleMax_Beta_WithParallelReduction3 <data_t,index_t><<<4096,512>>>(vec_d,Max_d,num_element,NSubranges,SubRangesize,alpha,SubrangeId_d,NSharedMemoryElements,beta);
		}
		else if (beta ==4)
		{
			//case beta =2
			...
			sampleMax_Beta_WithParallelReduction4 <data_t,index_t><<<4096,512>>>(vec_d,Max_d,num_element,NSubranges,SubRangesize,alpha,SubrangeId_d,NSharedMemoryElements,beta);
		}
