...
{
    cout<<"./exe exp_num_element k alpha scoreFile output_file"<<endl;
...
//     }
    ...
    if (NSubranges>k)
    {
        ...
        sampleMax<data_t,index_t><<<4096,512>>>(A_d,Max_d,N,NSubranges,SubRangesize,alpha,SubrangeId_d,Nthreadstowork);
        H_ERR(cudaDeviceSynchronize());
        t2=wtime();
        timeforMaxsample=t2-time_beg;
        //     cout<<"timeforMaxsample:"<<timeforMaxsample<<endl;
        //     cout<<"Max for every subranges"<<endl;
        bitonic_firstTopk<data_t,index_t>(Max_d,NSubranges,k,SubRangesize,NSubranges,SubrangeId_d,A_d,ConcatenatedRange_d,timeforFirstTopk,  concatenation_time);
        t3=wtime();
        // timeforFirstTopk=t3-t2;
        bitonic<data_t,index_t>(ConcatenatedRange_d,k*SubRangesize,k,TopKElement,SubRangesize,A,N,SubrangeId_d,A_d,ConcatenatedRange_d,timeforSecondTopk);
    }
    else
    {
        double NormalBitonicstart=wtime();
        bitonic<data_t,index_t>(A_d,N,k,TopKElement,SubRangesize,A,N,SubrangeId_d,A_d,ConcatenatedRange_d,timeforSecondTopk);
    ...
    cudaFree(Max_d);
    free(A);
    totalTime=timeforMaxsample+timeforFirstTopk+concatenation_time+timeforSecondTopk;
    cout<<"timeforMaxsample:"<<timeforMaxsample*1000<<" ms"<<endl;
    cout<<"timeforFirstTopk:"<<timeforFirstTopk*1000<<" ms"<<endl;
    cout<<"timeforConcatenation:"<<concatenation_time*1000<<" ms"<<endl;
   ...
   // timeLog.open("timeBitonicsampleOCT11.csv",std::fstream::out | std::fstream::app);
    timeLog.open(argv[4],std::fstream::out | std::fstream::app);
