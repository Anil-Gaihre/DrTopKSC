...
cout<<"Calculated alpha: "<<alpha<<endl;
	...
	//   for (int dis=3;dis<4;dis++)
	{
		...
		//    for (int Kiteration=atol(argv[2]);Kiteration<536870912;Kiteration=Kiteration*2)
		{
			...
			// timeLog.open("U_OptimumFICO_RADIX_VARY_N_N_2^30.csv",std::fstream::out | std::fstream::app);
			timeLog.open("U_OptimumFICO_RADIX_VARY_K_N_2^30.csv",std::fstream::out | std::fstream::app);
