...
cout<<"Calculated alpha: "<<alpha<<endl;
	...
	//   for (int dis=3;dis<4;dis++)
	{
		...
		//    for (int Kiteration=atol(argv[2]);Kiteration<536870912;Kiteration=Kiteration*2)
		{
			...
			data_t* vec1_d;
			H_ERR(cudaMalloc((void**) &vec1_d,sizeof(data_t)*num_element));
			H_ERR(cudaMemcpy(vec1_d,vec,sizeof(data_t)*num_element,cudaMemcpyHostToDevice));
			...
			timeLog.open("U_Vary_N_K_1024_4096.csv",std::fstream::out | std::fstream::app);
