...
void CumulateCount_inplace(index_t* Count,index_t* CumCount,index_t num_bucket,index_t& Kdigit,index_t k,index_t num_element,index_t Belowcount,data_t& flag, int NBitsperDigit,int Currentdigit)
{
	...
	cout<<"Inside CumulateCount_inplace()"<<endl;
...
__global__ void AssignKBucketElements(data_t* vec,index_t N,int Kdigit,int* countfortemp_d,data_t* temp,data_t flag,int CurrentDigit,int NBitsperDigit,int shift)
{
	...
	template<typename data_t, typename index_t>
__global__ void AssignKBucketElements_beta(data_t* vec,index_t N,int Kdigit,int* countfortemp_d,
	data_t* temp,data_t flag,int CurrentDigit,int NBitsperDigit,int shift,int beta,index_t* temp_subrangeId, 
	index_t* subrangeId,int* countKbucket,int* validSecondTopKCount,data_t* ConcatenatedRange_d, 
	index_t* CountLonelyElements_d, index_t* countSubRangeOccuraceCount_d)
{
	// extern __shared__ int SM_validSecondTopKCount[];
	// if (threadIdx.x==0) SM_validSecondTopKCount[0]=0;
	// __syncthreads();
	// index_t thid= (blockDim.x*blockIdx.x+threadIdx.x)*beta;
	index_t thid= (blockDim.x*blockIdx.x+threadIdx.x);
	while (thid<N)
	{
		int pos=thid;
		int count=0;
		data_t value=vec[pos];//Decide for the beta elements by the first element
		// data_t value1;
		// if (((value>>shift)<<shift)==flag)
		if (((value>>shift)<<shift)>=flag)
		{
			// count++;
			// value1 = vec[pos+1];//Designed for beta =2
			// count += (((value1>>shift)<<shift)==flag);

			// if (count == beta) 
			// {
				//Put the values into the new vector
				// int index=atomicAdd(countfortemp_d,beta);
				int index=atomicAdd(countfortemp_d,1);
				temp[index]=value;
				temp_subrangeId[index]=subrangeId[pos];
				// atomicAdd(countSubRangeOccuraceCount_d,1);
				// for (int i=1;i<beta;i++)
				// {
				// 	temp[index+i]=vec[pos+1];//Adding next consecutive beta elements for the evaluation
				// 	temp_subrangeId[index+i] = subrangeId[pos+1];
				// }
			// }
			// else
			// {
			// 	//only the largest values satisfies. Put it into lonely value category (both for simplicity)
			// 	index_t curr_ptr = atomicAdd(CountLonelyElements_d,beta);
			// 	atomicAdd(validSecondTopKCount,1);
			// 	ConcatenatedRange_d[curr_ptr]=value;
			// 	ConcatenatedRange_d[curr_ptr + 1]=value1;
			// }


		}
		// else if(((value>>shift)<<shift) < flag)
		// {//added for beta implementation
		// 	//if maximum of beta element doesn't fall in the k bucket
		// 	atomicAdd(&SM_validSecondTopKCount[0],beta);
		// }
		// thid+=(blockDim.x*gridDim.x)*beta;
		thid+=(blockDim.x*gridDim.x);
	}
	// if (threadIdx.x==0) atomicAdd(validSecondTopKCount,SM_validSecondTopKCount[0]);
}
...

{
	...
	template<typename data_t, typename index_t>
__global__ void CalculateOccurence_inplace_firsttopk_beta(data_t* vec,index_t num_element,index_t* Count,int NBitsperDigit,int CurrentDigit,int num_bucket,data_t flag, int shiftleft,int shiftRight,int beta)

{
	...
	while(mybegin < num_element)
	{
		// int pos=mybegin*beta;//Decide on the basis of max value among the beta elements 
		int pos=mybegin;//Decide on the basis of max value among the beta elements 
		data_t value=vec[mybegin];
		if (((value>>shiftleft)<<shiftleft)==flag)//The value falls within the bucket
		{
			data_t temp=value>>shiftRight;
			temp=temp&mask;//Identifying the bucket
			// atomicAdd(&SMem_Count[temp],beta);
			atomicAdd(&SMem_Count[temp],1);
		}
		// mybegin+=(blockDim.x*gridDim.x)*beta;
		mybegin+=(blockDim.x*gridDim.x);
	}
	__syncthreads();
	for (int i = threadIdx.x; i < num_bucket; i += blockDim.x) 
...
template<typename data_t,typename index_t>  
__global__ void findEligibleSubranges(data_t* vec_d,index_t num_element,data_t flag,int shiftleft,index_t* SubrangeId_d,index_t* SelectedSubrangeId_d,index_t* CountSelectedSubrange_d,data_t* ConcatenatedRange,index_t* CountLonelyElements,int beta,index_t* countSubRangeOccuraceCount_d)
{
	index_t mybegin=(blockIdx.x*blockDim.x+threadIdx.x);//*3 for Beta
	while(mybegin < num_element)
	{
		data_t value=vec_d[mybegin];
		if (value >= flag)
		{
			index_t subrangeId = SubrangeId_d[mybegin];
			atomicAdd(&countSubRangeOccuraceCount_d[subrangeId],1);
		}
		mybegin+=blockDim.x*gridDim.x;//*3 for beta
	}
}

template<typename data_t,typename index_t>  
__global__ void FindTopKRanges_beta_with_outplace(data_t* vec_d,index_t num_element,data_t flag,int shiftleft,index_t* SubrangeId_d,index_t* SelectedSubrangeId_d,index_t* CountSelectedSubrange_d,data_t* ConcatenatedRange,index_t* CountLonelyElements,int beta,index_t* countSubRangeOccuraceCount_d)

{
	...
	switch(beta)
	{
		case 2:		
			...
			{
				int count=0;
				index_t my_subrangeId= SubrangeId_d[mybegin];
				int countOccurace = atomicCAS(&countSubRangeOccuraceCount_d[my_subrangeId], beta, 0);
				if (countOccurace == beta)
				{
					//called only once. As the count is replaced by 0 in above logics
					SelectedSubrangeId_d[atomicAdd(CountSelectedSubrange_d,1)]=my_subrangeId;
					// printf("Eligible subrange: %d\n",my_subrangeId);
				}
				else if (countOccurace == beta-1)
				{
					//Only this value is suitable for topk
					//Add as a lonely element
					index_t curr_ptr = atomicAdd(CountLonelyElements,1);
					ConcatenatedRange[curr_ptr]=vec_d[mybegin];
				}
				mybegin+=blockDim.x*gridDim.x;//*3 for beta
			}
			break;
			default:
			printf("A section in radix select is skipped. Please assign elibile beta!\n");
			break;
	}
}
...
void radix_select_inplace_firsttopk(data_t* vec_d,index_t num_element,index_t k,index_t num_bucket,data_t& TopKElement,int NBitsperDigit,int CurrentDigit,data_t flag,index_t* SubrangeId_d,index_t* SelectedSubrangeId_d,index_t* CountSelectedSubrange_d)
{
	...
	while(CurrentDigit>=0)
	{
		...
		H_ERR(cudaMemcpy(Count,Count_d,sizeof(index_t)*num_bucket,cudaMemcpyDeviceToHost));
		memset(CumCount, 0, num_bucket * sizeof(index_t));
		//Finding the below count and the KthBucket
		cout<<"Before calling CumulateCount_inplace() in radix_select_inplace_firsttopk()"<<endl;
		CumulateCount_inplace<data_t,index_t>(Count,CumCount,num_bucket,Kdigit,k,num_element,Belowcount,flag,NBitsperDigit,CurrentDigit);
		if (Kdigit!=0)  Belowcount=CumCount[Kdigit-1];
		cout<<"KDigit"<<Kdigit<<endl;
...
{
	double inplace_start=wtime();
	cout<<endl<<endl<<"Inside InPlace Radix Select(Phase II) BETA Version"<<endl;
	index_t Kcount;
	...
	while(CurrentDigit>=0)
	{
		...
		shleft=(CurrentDigit+1)*(NBitsperDigit);
		shright=CurrentDigit*NBitsperDigit;
		CalculateOccurence_inplace_firsttopk_beta<data_t,index_t><<<128,128,num_bucket*sizeof(index_t)>>>(vec_d,num_element,Count_d,NBitsperDigit,CurrentDigit,num_bucket,flag,shleft,shright,beta);
		H_ERR(cudaDeviceSynchronize());
		cout<<"After CalculateOccurence_inplace_firsttopk() module"<<endl;
		H_ERR(cudaMemcpy(Count,Count_d,sizeof(index_t)*num_bucket,cudaMemcpyDeviceToHost));
		memset(CumCount, 0, num_bucket * sizeof(index_t));
		cout<<"Before calling CumulateCount_inplace() in radix_select_inplace_firsttopk_beta()"<<endl;
		CumulateCount_inplace<data_t,index_t>(Count,CumCount,num_bucket,Kdigit,k,num_element,Belowcount,flag,NBitsperDigit,CurrentDigit);
		if (Kdigit!=0)  Belowcount=CumCount[Kdigit-1];
		cout<<"KDigit"<<Kdigit<<endl;
		cout<<"BelowCount:"<<Belowcount<<endl;
...
template<typename data_t, typename index_t>
void radix_select_inplace_firsttopk_beta_with_outplace(data_t* vec_d,index_t num_element,index_t k,
	index_t num_bucket,data_t& TopKElement,int NBitsperDigit,int CurrentDigit,data_t flag,
	index_t* SubrangeId_d,index_t* SelectedSubrangeId_d,index_t* CountSelectedSubrange_d,
	data_t* ConcatenatedRange_d,index_t* CountLonelyElements_d,int beta, index_t* countSubRangeOccuraceCount_d)
{
	double inplace_start=wtime();
	cout<<endl<<endl<<"Inside InPlace Radix Select(Phase II) BETA Version"<<endl;
	index_t Kcount;
	...
	shleft=(CurrentDigit+1)*(NBitsperDigit);
	findEligibleSubranges<data_t,index_t><<<128,128>>> (vec_d,num_element,flag,shleft,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d,ConcatenatedRange_d,CountLonelyElements_d,beta,countSubRangeOccuraceCount_d);	
	FindTopKRanges_beta_with_outplace<data_t,index_t><<<128,128>>> (vec_d,num_element,flag,shleft,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d,ConcatenatedRange_d,CountLonelyElements_d,beta,countSubRangeOccuraceCount_d);
	H_ERR(cudaDeviceSynchronize());
	TopKElement=flag;
	//  int count;
	//  H_ERR(cudaMemcpy(&count,CountSelectedSubrange_d,sizeof(int),cudaMemcpyDeviceToHost));
	...
	template<typename data_t, typename index_t>
void radix_select(data_t* vec_d,index_t num_element,index_t k,index_t num_bucket,data_t& TopKElement,int NBitsperDigit,int CurrentDigit, bool defaultContribution,index_t* SubrangeId_d,index_t* SelectedSubrangeId_d,index_t*  CountSelectedSubrange_d,data_t* ConcatenatedRange_d, index_t* CountLonelyElements_d,int beta,int NSubranges)
{
	...
	if (num_element<= limit)
	{
		...
		{
			// NSubranges =  num_element;
			radix_select_inplace_firsttopk<data_t,index_t>(vec_d,num_element,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,flag,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d);
		}
		else
		{
			radix_select_inplace_firsttopk_beta<data_t,index_t>(vec_d,num_element,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,flag,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d,ConcatenatedRange_d,CountLonelyElements_d,beta);
		}
	...
	index_t* temp_subrangeId_d;
	H_ERR(cudaMalloc((void**) &temp_subrangeId_d,sizeof(index_t)*num_element));
	//  index_t* subrangeId_d;
	//    num_bucket=1<<NBitsperDigit;
	index_t* countfortemp_d;
	cudaMalloc((void**) & countfortemp_d,sizeof(index_t));

	int countKbucket=0;//added for beta implementation
	int validSecondTopKCount=0;//added for beta implementation
	int* countKbucket_d;//added for beta implementation
	int* validSecondTopKCount_d;//added for beta implementation
	cudaMalloc((void**) & countKbucket_d,sizeof(int));//added for beta implementation
	cudaMalloc((void**) & validSecondTopKCount_d,sizeof(int));//added for beta implementation
	index_t* countSubRangeOccuraceCount_d;
	H_ERR(cudaMalloc((void**) & countSubRangeOccuraceCount_d,NSubranges*sizeof(index_t)));//added for beta implementation
	H_ERR(cudaMemset(countSubRangeOccuraceCount_d, 0, NSubranges * sizeof(index_t)));

	// while((n>limit) && (CurrentDigit>=0))
	{

		cout<<endl<<endl<<"Processing CurrentDigit:"<<CurrentDigit<<"in Out-place radix select!"<<endl;
		cout<<"Current vector size: "<<n;
		Kcount=0;
		H_ERR(cudaMemset(Count_d, 0, num_bucket * sizeof(index_t)));
		int shright=CurrentDigit*NBitsperDigit;
		...
		// AssignKBucketElements<data_t,index_t><<<128,128>>>(vec_d,n,Kdigit,countfortemp_d,temp_d,flag,CurrentDigit,NBitsperDigit,shift,beta);
		//copies the eligible elements to the new vector
		//Currently I'm copying all the beta elements to the new vector in AssignKBucketElements_beta
		H_ERR(cudaMemset(countKbucket_d, 0, sizeof(int)));
		H_ERR(cudaMemset(validSecondTopKCount_d, 0, sizeof(int)));
		AssignKBucketElements_beta<data_t,index_t><<<128,128,sizeof(int)>>>(vec_d,n,Kdigit,countfortemp_d,temp_d,flag,CurrentDigit,NBitsperDigit,shift,beta,temp_subrangeId_d, SubrangeId_d,countKbucket_d,validSecondTopKCount_d, ConcatenatedRange_d, CountLonelyElements_d,countSubRangeOccuraceCount_d);
		H_ERR(cudaDeviceSynchronize());
		index_t countfortemp;//Size of the new vector
		H_ERR(cudaMemcpy(&countfortemp,countfortemp_d,sizeof(int),cudaMemcpyDeviceToHost));
		// H_ERR(cudaMemcpy(&countKbucket,countKbucket_d,sizeof(int),cudaMemcpyDeviceToHost));
		// H_ERR(cudaMemcpy(&validSecondTopKCount,validSecondTopKCount_d,sizeof(int),cudaMemcpyDeviceToHost));
		...
		// n=Count[Kdigit];
		n=countfortemp;//#values in the k bucket
		// n=countfortemp;//#values in the k bucket
		cout<<"Kcount: "<<Kcount<<endl;
		cout<<"New k: "<<k<<endl;
		cout<<"New num_element: "<<n<<endl;

		swap_ptr_data<data_t,index_t>(temp_d, vec_d);
		swap_ptr_index<data_t,index_t>(temp_subrangeId_d, SubrangeId_d);

		if (Kcount==1)
		{//This won't be true for beta>1 implementation
	...
	if (defaultContribution)
	{
		radix_select_inplace_firsttopk<data_t,index_t>(vec_d,n,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,flag,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d);
	}
	else
	{
		cout<<"Calling radix_select_inplace_firsttopk_beta_with_outplace() module"<<endl;
		radix_select_inplace_firsttopk_beta_with_outplace<data_t,index_t>(vec_d,n,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,flag,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d,ConcatenatedRange_d,CountLonelyElements_d,beta,countSubRangeOccuraceCount_d);
	}
	// radix_select_inplace<data_t,index_t>(vec_d,n,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,flag);
	H_ERR(cudaFree(temp_subrangeId_d));
	H_ERR(cudaFree(temp_d));
}
...
void sample_radix_select(data_t* vec_d,index_t num_element,index_t k,index_t num_bucket,data_t& TopKElement,int NBitsperDigit,int CurrentDigit,index_t NSubranges,index_t SubRangesize,index_t alpha,double& timeforMaxsample,double& timeforFirstTopK,double& timeForsecondTopK,double& timeforConcatenation,data_t* Max_d,index_t* SubrangeId_d,int NSharedMemoryElements,int SizeOfSubWarp,int pow_size_Subwarp,index_t NSubWarps_InBlock,int NSubRangesPerBlock,int NElementsPerBlock_ReadFromGlobal,int
		TotalBlocksrequired,int SizeOfAllocation,int NThreadsPerBlock,int beta,bool defaultContribution,int NthreadstoworkInreduction,index_t* SelectedSubrangeId_d,index_t*CountLonelyElements_d,index_t* write_pos_d,data_t* ConcatenatedRange_d,index_t* CountSelectedSubrange_d,data_t* original_vec_d)
{
	...
	else
	{
		if (beta ==2)
		{
			cout<<"Calling beta=2 module for sampling!"<<endl;
	...
	if (defaultContribution)
	{
		radix_select<data_t,index_t>(Max_d,NSubranges,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,defaultContribution,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d,ConcatenatedRange_d,CountLonelyElements_d,beta,NSubranges);
	...
	{
		radix_select<data_t,index_t>(Max_d,NSubranges*beta,k,num_bucket,TopKElement,NBitsperDigit,CurrentDigit,defaultContribution,SubrangeId_d,SelectedSubrangeId_d,CountSelectedSubrange_d,ConcatenatedRange_d,CountLonelyElements_d,beta,NSubranges);
	...
	if (defaultContribution)
	{
		...
		CreateConcatenateRange_optimized<data_t,index_t><<<512,512>>>(original_vec_d,ConcatenatedRange_d,SelectedSubrangeId_d,ConcatenatedSize,k,SubRangesize,write_pos_d,TopKElement);
	...
	else
	{
		...
		CreateConcatenateRange_Beta_optimized<data_t,index_t><<<512,512>>>(original_vec_d,ConcatenatedRange_d,SelectedSubrangeId_d,ConcatenatedSize,count,SubRangesize,CountLonelyElements_d,write_pos_d,TopKElement);
